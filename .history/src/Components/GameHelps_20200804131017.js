...
export default function GameHelps({names, numGioc, tipo, myTournament, setMyTournament, switchTracks, setSwitchTracks, currentGameId }) {
    ...
    const decrementSwitch = (i) => {
        const temp = [...switchTracks];
        let intTrack = getRandomInt(0, temp.length);
        let choice = temp[intTrack];
        temp.splice(intTrack, 1);
        setSwitchTracks(temp);
        //{ index: i + 1, num: choice.id, name: choice.name, type: 'track', state: 'uncomplete', points: Array(numGioc).fill(null) };
        //deve cambiare la gara corrente
