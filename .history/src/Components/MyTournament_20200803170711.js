...
export default function MyTournament({ tipo, numGioc, battles, tracks, players, setPlayers }) {
    ...
    const getWinner = () => {
        let tempWinners = [players[0].name];
        tempPts = players[0].getTotalPoints();
        players.forEach(pl=>{
            plTot = pl.getTotalPoints();
            if( plTot > tempPts ){
        ...
        if(tempWinners.length > 1){
            ...
            return "Torneo concluso in parità, i vincitori sono: "
        }
    }
    useEffect(() => {
        ...
        <>
            ...
            {((end + 1) === (currentGameId)) && (
                <div className="col-12">
                    <p>Vince il torneo: {getWinner()}</p>
                    <p>Vince il premio battaglia: {getBtWinner()}</p>
                    <p>Vince il premio gara: {getTrWinner()}</p>
                </div>
            )}
        </>
    )
