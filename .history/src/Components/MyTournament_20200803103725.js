    ...
    useEffect(() => {
        ...
        setMyTournament(Array(temp).fill(null).map((el, i) => {
            ...
            if (i === 0){
                ...
                return { index: i+1, num: intBattle, choice: choice, type: 'battle', state: 'uncomplete', points: Array() };
            }
            else if (i>0 && i<=3){
                let intTrack = getRandomInt(0, tempTracks.length);
                let choice = tempTracks[intTrack];
                tempTracks.splice(intTrack,1);
                return { index: i+1, num: intTrack, choice: choice, type: 'track', state: 'uncomplete' };
            }
