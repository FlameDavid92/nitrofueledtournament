...
export default function MyTournament({ tipo, numGioc, battles, tracks, players }) {
    ...
    useEffect(() => {
        ...
        setMyTournament(Array(temp).fill(null).map((el, i) => {
            ...
            if (i === 0){
                ...
                //let pref = randomPref[i-4];
                let pref = {num:0,name:"0",player:"pl"};
                //randomizzare le preferite
                return { index: i+1, num: pref.num, name: pref.name+" ["+pref.player+"]", type: 'track', state: 'uncomplete', points: Array(numGioc).fill(null) };
            }
            else{
                console.log("battaglia random - "+tipo);
                let intBattle = getRandomInt(0, tempBattles.length);
    ...
    return (
        <>
            {myTournament && (
                <ul>
                    {myTournament.map((el,i)=><li key={'tm-'+i}>{el.index+". "+el.name+" ("+el.num+") - "+(el.type === 'battle') ? 'BATTAGLIA' : 'GARA'}</li>)}
                </ul>
            )}
        </>
    )
