...
export default function InitForm2({ tracks, numGioc, setPlayers, favoriteTracks, setFavoriteTracks, setInitForm2 }) {
    ...
    function filterTracks(idTr,i){
        console.log("trid: "+idTr);
        console.log("i: "+i);
        console.log("fun: "+favoriteTracks.some((fvTr, fvInd) => {return (idTr !== fvTr || fvInd === i)}));
        return favoriteTracks.some((fvTr, fvInd) => {return (idTr !== fvTr || fvInd === i)});
    }
                                ...
                                return (
                                    <div key={"input-" + (i + 1)} className="mt-2">
                                        ...
                                        <select name={"inputStatePlayer-" + (i + 1)} id={"inputStatePlayer-" + (i + 1)}
                                            ...
                                            {tracks.filter((tr)=>filterTracks(tr.id,i)).map(tr => {
