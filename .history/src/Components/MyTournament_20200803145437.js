...
export default function MyTournament({ tipo, numGioc, battles, tracks, players }) {
    ...
    return (
        <>
            {myTournament && (
                <ul>
                    {myTournament.map((el, i) => <li className={el.type+" "+el.state+((currentGameId === i) ? ' current':'')} key={'tm-' + i}>{el.index + ". " + el.name + " (" + el.num + ") - " +
                        ((el.type === 'battle') ? 'BATTAGLIA' : ((el.type === 'favorite') ? 'PREFERITA' : 'GARA'))}</li>)}
                </ul>
            )}
            <PointsManager players={players} ></PointsManager>
