                  ...
                  {Array(parseInt(numGioc)).fill(null).map((pl, i) => {
                    return (
                      <div key={"input-" + (i + 1)} className="mt-2">
                        ...
                        <label htmlFor={"inputStatePlayer-"+(i+1)}>Numero giocatori:</label>
                        <select name={"inputStatePlayer-"+(i+1)} id={"inputStatePlayer-"+(i+1)} className="form-control text-center" defaultValue="" required>
                          <option disabled hidden value="">{"Gara preferita del giocatore " + (i + 1)}</option>
                          {nitroFueled.tracks.map(tr => {
                            return (<option>{tr.id}. {tr.name}</option>)
                          })}
                        </select>
