...
class Player{
  constructor(name, battlePoints, racePoints){
    this.name = name;
...
function App() {
  ...
  const inizializzaTorneo = (e) => {
    ...
    setInitForm(false);
    setPlayers(Array(parseInt(numGioc)).fill(Player("",0,0)));
    return;
  }
              ...
              <div className="form-row text-center">
                <div className="form-group col-12">
                  {Array(parseInt(numGioc)).fill(null).map((pl, i) => {
                    return (
                      <div className="mt-2">
                        <input type="text" name={"playerName-" + (i+1)} id={"playerName-" + (i+1)} placeholder={"Nome giocatore " + (i+1)} value={} required></input>
                      </div>)}
                  )}
                  <input className="btn btn-outline-primary mt-2" type="submit" value="Avanti" />
                </div>
